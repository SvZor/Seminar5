...
class UserInputToCompileForTest
{
    ...
    {   
       //Напишите свое решение здесь
      // Вызов метода для вычисления суммы по строкам
        int[] sums = SumRows(numbers);
        // Вызов метода для получения индекса строки с минимальной суммой
        int minIndex = MinIndex(sums);
        Console.WriteLine(minIndex); // Вывод индекса строки с наименьшей суммой элементов
    }
}
